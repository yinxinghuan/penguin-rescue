...
Penguin Rescue · 救小企鹅 是一个 `action` 类型的移动端小游戏：Save the babies. Dodge the skua. — 摇杆拖动母企鹅穿过浮冰，把走失小企鹅一只只接成长龙；天上的南极贼鸥会朝你俯冲，碰一下就 game over

## 2. Visual Design

- 整体布局：页面占用 100vw x 100vh，主体验居中，HUD 与操作区覆盖在游戏层上方，移动端以单手操作为优先。
- 背景与配色：主要颜色使用 #1f3d5a、#1c3650、#6996b6、#f4fbff、#FFD700、#ffe17a、#000、#465464；高亮元素用于可点击目标、得分、结果或稀有状态。
- 字体：使用 'Space Grotesk', 'Inter', system-ui, sans-serif、'Baloo 2', 'Space Grotesk', system-ui, sans-serif、'JetBrains Mono', monospace、'Baloo 2', sans-serif，按钮与状态文字保持 12-24 px 的可读范围。
...
- `PLAYER_SPEED`：12
- `SKUA_BASE_SPEED`：3.5
- `ICEBERG_AVOID_RADIUS`：1.2
- `INITIAL_ICEBERGS`：8
- `MAX_BABIES`：15
- `SPAWN_INTERVAL_MIN`：1
- `SPAWN_INTERVAL_MAX`：3
- `CATCH_RADIUS`：2
- `THREAT_HIT_RADIUS`：1.5
- `BODY_FOLLOW_SPEED`：3
- `SEGMENT_SIZE_BOOST`：0.02
- `SEAL_SPAWN_INTERVAL`：5
- `SEAL_MAX`：3
